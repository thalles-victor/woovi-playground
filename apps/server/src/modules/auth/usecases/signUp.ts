...
import { SignUpDto, signUpDtoSchema } from "./auth.dtos";
...
export async function signUpUseCase(signUpDto: SignUpDto) {
  validateSchema(signUpDto, signUpDtoSchema );
    ...
    try {
      ...
      const token = await JWT.sign(payload, envParsed.JWT_SECRET, {
